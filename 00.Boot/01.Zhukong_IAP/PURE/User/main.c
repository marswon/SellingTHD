    ...
    else
    {
        while(1)
        {
            if(USART_BufferRead(&data) == 1)
            {
                ...
                else if(start_flash_flag == Start_Flash_Bod)  // 开始升级主控
                {
                    IAP_Start_Program_Flash(data);
                }
                else if(start_flash_flag == Stop_Flash_Bod) // 结束升级主控
                {
                    if(flag_dis_jump == 0)
                    {
                        flag_dis_jump = 1;
                        ...
                        //Send_CMD(USART2, 0x01, LBYTE(USARTCMD_ANDROID_ZHUKONG_StopUpdateZhukong));
                        //for(i = 0; i < 30 * 1024; i = i + 4)
                        //{
                        //memset(strstr, 0, sizeof(strstr));
                        //sprintf(strstr, "%08x ", *(__IO uint32_t*)(FLASH_APP_ADDRESS + i));
                        //USART_SendBytess(USART2, strstr);
                        //}
