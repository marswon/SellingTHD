...
void USART_BufferWrite(u8 ntemp)
{
            ...
            && UsartBuffer[(USART_BUFFER_LEN + UsartWptr - 6) % USART_BUFFER_LEN] == 0x12 && UsartBuffer[(USART_BUFFER_LEN + UsartWptr - 7) % USART_BUFFER_LEN] == 0x01)
    {
        ...
        USART_SendBytess(USART1, "sys reset \r\n");
        __disable_irq();
        NVIC_SystemReset();
    }
    ...
    if(UsartBuffer[UsartRptr] == 0x0A && UsartBuffer[(USART_BUFFER_LEN + UsartRptr - 1) % USART_BUFFER_LEN] == 0x0D
            && UsartBuffer[(USART_BUFFER_LEN + UsartRptr - 2) % USART_BUFFER_LEN] == 0xE0 && UsartBuffer[(USART_BUFFER_LEN + UsartRptr - 3) % USART_BUFFER_LEN] == 0x2A
            ...
            && UsartBuffer[(USART_BUFFER_LEN + UsartRptr - 6) % USART_BUFFER_LEN] == 0x4E && UsartBuffer[(USART_BUFFER_LEN + UsartRptr - 7) % USART_BUFFER_LEN] == 0x01)
    {
        start_flash_flag = Stop_Flash_Bod;  //结束主控板升级
    }
}
...
void Handle_USART_CMD(u16 Data, char *Dat, u16 dat_len)
{
    ...
    {
        if(Data == USARTCMD_ANDROID_ZHUKONG_WillUpdateZhukong) // will升级主控
        {
            flag_dis_jump = 1;
            Send_CMD(USART3, HBYTE(USARTCMD_ANDROID_ZHUKONG_WillUpdateZhukong), LBYTE(USARTCMD_ANDROID_ZHUKONG_WillUpdateZhukong));
            memset(strtmp, 0, sizeof(strtmp));
            sprintf(strtmp, "will update: %04X\r\n", USARTCMD_ANDROID_ZHUKONG_WillUpdateZhukong);
            USART_SendBytess(USART1, strtmp);
        }
        else if(Data == USARTCMD_ANDROID_ZHUKONG_StartUpdateZhukong) // 开始升级主控
        {
            flag_dis_jump = 0;
            IAP_Start_Program_Flash_Init();
            start_flash_flag = Start_Flash_Bod;
            Send_CMD(USART3, HBYTE(USARTCMD_ANDROID_ZHUKONG_StartUpdateZhukong), LBYTE(USARTCMD_ANDROID_ZHUKONG_StartUpdateZhukong));
            memset(strtmp, 0, sizeof(strtmp));
            sprintf(strtmp, "start update: %04X\r\n", USARTCMD_ANDROID_ZHUKONG_StartUpdateZhukong);
            USART_SendBytess(USART1, strtmp);
        }
        else if(Data == USARTCMD_ANDROID_ZHUKONG_GetZhukongVer) // 获取主控版本
        {
            char strstr[10] = {0};
        ...
        else if(Data == 0x0117) // 初始化flash标志位，打开进入APP
        {
            ...
            USART_SendBytess(USART1, "init Zhukong\r\n");
            __disable_irq();
            NVIC_SystemReset();
        }
    }
